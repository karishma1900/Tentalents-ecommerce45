...
export const getAllProducts = async () => {
  const res = await fetch(`https://product-service-ydpt.onrender.com/api/products`);
  if (!res.ok) {
...
export const getProductBySlug = async (slug: string) => {
  const res = await fetch(`https://product-service-ydpt.onrender.com/api/products/slug/${encodeURIComponent(slug)}`);
